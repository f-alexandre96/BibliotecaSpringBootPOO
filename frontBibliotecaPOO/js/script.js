...
async function fetchAndRenderUsers() {
    try {
        const response = await fetch('http://localhost:8090/livros');
...
async function deleteUser(id) {
    ...
    try {
        const response = await fetch(`http://localhost:8090/livros/${id}`, { method: 'DELETE' });
...
if (editForm) {
    editForm.addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch(`http://localhost:8090/livros/${userId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
